...
sum(primes) // => 28: sum of the first 5 primems 2+3+5+7+11

function factorial(n){  // A function to compute factorials
    let product = 1;    // Start with a product of 1
    while(n>1){ //Repeat statements if {} while expr in () is true
...
class Point{    // By convention, class names are capitalized
    constructor(x,y){   // Constructor functioin to initialize new instances
        this.x = x; // This keyword is the new object being initialized
        this.y = y; // Store function arguments as object properties
    }   // No return is necessary in constructor functions
    
    distance(){ // Method to compute distance from origin to point
        return Math.sqrt(   // Return the square root of x^2 to y^2
        this.x*this.x + // this refers to teh Point object on which
