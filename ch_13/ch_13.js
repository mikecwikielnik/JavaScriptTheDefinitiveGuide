...
function clock(interval, max = Infinity){
    // A Promise-ified version of setTimeout that we can use awaith with.
 ...
 * and remove them dequeue(). dequeue() returns a Promise, which
 * means that values can be dequeued before tehy are enqueued. The
...
class AsyncQueue{
    constructor(){
        ...
        this.resolvers = [];
        // Once closed, no more values can enqueued, and no more unfulfilled
        // Promises returned.
        this.closed = false;
    }
    ...
    // Define the method that makes this an asynchronously iterator. The
